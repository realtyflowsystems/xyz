...
// Triggered by booking.html form (no JWT required — public endpoint).
// Actions: upsert lead → create booking → send immediate confirmation email
//          → enroll in Post-Booking Nurture sequence (SMS reminder fires via sequence-runner)
...
// Stage constants matching the actual DB schema
const STAGE_BOOKED = 2;
const STAGE_NAME_BOOKED = "Booked";
...
serve(async (req: Request) => {
  ...
  try {
    ...
    const { name, email, phone, slot_time, source } = body;
    ...
    // Upsert lead by email — update stage if they're booking for the first time or re-booking
    ...
    if (resendKey) {
      const slotDisplay = slot_time
        ? new Date(slot_time).toLocaleString("en-US", {
      ...
      const emailRes = await fetch("https://api.resend.com/emails", {
        ...
        body: JSON.stringify({
          ...
          html: confirmationEmail(fname, slotDisplay),
        }),
      });

      const emailData = await emailRes.json();

      // Log email in the `emails` table
    ...
    // Enroll in Post-Booking Nurture sequence (handles 24hr SMS reminder)
    // Find the sequence by name
    const { data: sequence } = await supabase
      .from("sequences")
      .select("id")
      .eq("name", "Post-Booking Nurture")
      .eq("active", true)
      .single();

    if (sequence) {
      // Get step 2 (SMS reminder at 23hrs) — we skip step 1 since we sent confirmation directly
      ...
      await supabase.from("sequence_enrollments").insert({
        ...
        current_step: 1, // step 1 already handled by this function
        next_send_at: step2
          ? new Date(Date.now() + step2.delay_hours * 3_600_000).toISOString()
          : null,
    ...
    await supabase.from("activity_log").insert({
      ...
      description: `Revenue Audit booked${slot_time ? " for " + slot_time : ""}`,
      metadata: { booking_id: booking.id, source: source ?? "Booking Page" },
    });
...
function confirmationEmail(fname: string, slotDisplay: string | null): string {
  const timeBlock = slotDisplay
    ? `<div style="background:#111;border-left:3px solid #C9A84C;padding:16px 20px;margin:24px 0;">
         ...
         <p style="font-size:18px;color:#fff;font-weight:300;margin:0;">${slotDisplay}</p>
       </div>`
    : `<p style="color:#888;font-size:13px;line-height:1.7;margin:16px 0;">We'll reach out within 24 hours to lock in your time.</p>`;
...
<body style="background:#0A0A0A;color:#fff;font-family:'Helvetica Neue',Arial,sans-serif;max-width:560px;margin:0 auto;padding:0;">
  ...
  <div style="padding:32px 40px;">
    <p style="font-size:15px;color:#E0E0E0;margin:0 0 12px;">Hi ${fname},</p>
    <p style="font-size:14px;color:#AAA;line-height:1.75;margin:0 0 16px;">
      You've secured your Revenue Audit. This call will show you exactly how much revenue is slipping through your lead flow — and what recovery looks like in your market.
    </p>
    ${timeBlock}
    <div style="background:#111;border:1px solid rgba(201,168,76,.15);padding:24px;margin:24px 0;">
      ...
      <ul style="color:#AAA;font-size:13px;line-height:2;padding-left:18px;margin:0;">
        <li>Monthly lead volume (Zillow, referral, organic)</li>
    ...
    <p style="font-size:13px;color:#AAA;line-height:1.75;">
      Questions? Reply here or reach me at
  ...
  <div style="padding:20px 40px;border-top:1px solid rgba(255,255,255,.06);">
    <p style="font-size:11px;color:#444;margin:0;line-height:1.7;">
      ...
      MA TCPA Compliant · <a href="mailto:erics@realtyflow.xyz?subject=Unsubscribe" style="color:#555;">Unsubscribe</a>
    </p>
  </div>
</body>
</html>`;
